...
if (isset($_POST['update'])) {
    ...
    if ($image != '') {
        ...
        $updateQuery = "UPDATE product SET ProductImage='$image', ProductName='$name', ProductDes='$description', ProductPrice='$price' WHERE ProductID = $id";
    } else {
        // Update query without image (keep the old one)
        $updateQuery = "UPDATE product SET ProductName='$name', ProductDes='$description', ProductPrice='$price' WHERE ProductID = $id";
    }
...
//  Insert ( functionality for adding new products)
if ($_SERVER['REQUEST_METHOD'] == 'POST' && !isset($_POST['update'])) {
    ...
    if ($imageCheckResult->num_rows > 0) {
        ...
        if (move_uploaded_file($_FILES['image']['tmp_name'], $target_file)) {
            ...
            $insertQuery = "INSERT INTO product (ProductImage, ProductName,  ProductDes, ProductPrice) 
